...
const handlePrismaError = (error) => {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
        switch (error.code) {
            case "P2002": {
                const field = error.meta?.driverAdapterError?.cause?.constraint
                    ?.fields?.[0] ?? error.meta?.target?.[0];
    ...
    const uniqueInputMatch = message.match(/Argument `(\w+)` of type .* needs at least one of `(.+?)` arguments/);
    if (uniqueInputMatch) {
